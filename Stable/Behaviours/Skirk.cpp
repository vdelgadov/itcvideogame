...
class SkirkEngaging : public AState{
private:

	const double attacking_range = 1.5;
public:
	void enter(Actor* a){ 
...
class SkirkAttack : public AState {
	...
	void execute(Actor* a){
		...
		a->getVehicle()->setCurrVel(stf.normalize()*(a->getVehicle()->getMaxSpeed()+0.2));
		cout << "Chargin  >> " << endl;
